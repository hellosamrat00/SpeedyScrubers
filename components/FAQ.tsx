      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-start">
        ...
        <div>
          ...
          <h2 className="font-heading text-4xl md:text-5xl font-bold leading-tight mb-6" style={{ color: "var(--color-brand-blue)" }}>
            You Have{" "}
            <span style={{ color: "var(--color-brand-yellow)" }}>Questions,</span>
            <br />
            We Have Answers
          </h2>
        ...
        <div className="pt-8 space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="faq-item rounded-xl p-6 bg-white shadow-sm border transition-all" style={{ borderColor: open === i ? "var(--color-brand-yellow)" : "transparent" }}>
              <button
                className="w-full flex items-center justify-between text-left gap-4 group"
                onClick={() => setOpen(open === i ? null : i)}
              >
                <span className="font-semibold transition-colors" style={{ color: open === i ? "var(--color-brand-blue)" : "var(--color-brand-blue)", opacity: open === i ? 1 : 0.9 }}>
                  {faq.q}
                </span>
                <span className="text-2xl flex-shrink-0 transition-transform" style={{ color: open === i ? "var(--color-brand-yellow)" : "rgba(5,38,27,0.3)", transform: open === i ? "rotate(45deg)" : "none" }}>+</span>
              </button>
              <div 
              ...
              >
                <div className="overflow-hidden">
                  <p className="text-sm leading-relaxed pr-8" style={{ color: "var(--color-brand-blue)", opacity: 0.7 }}>{faq.a}</p>
                </div>
              </div>
            </div>
          ))}
